/**
 * Modular calendar + dated wellbeing backfill helpers.
 * Google Calendar remains the source of truth; this endpoint only controls
...
function getCalendarViewV4(request, force) {
  ...
  const calendarMeta = allCalendars.map(function (cal) {
    return {
      id: cal.getId(),
      name: cal.getName() || '(ohne Namen)',
      color: cal.getColor() || '#1B4632',
      defaultVisible: !/möglichkeit/i.test(cal.getName() || '')
    };
  });
  ...
  return cachedJson_(cacheKey, 60, !!force, function () {
    ...
    allCalendars.forEach(function (cal) {
      ...
      try {
        ...
        // One inaccessible calendar must not break the complete calendar tile.
      }
    });
...
/**
 * V2 keeps V1's audited upsert behavior, but allows an explicit historical
 * entry date and rejects future entries.
 */

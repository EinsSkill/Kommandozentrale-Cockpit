...
test('Claude Design source and runtime remain traceable and design-locked', () => {
  assert.match(index, /Claude Design source SHA-256: 01b102d2f95fef3b037030d946e4a9ab98a819eb3926ca3d35250c8c1bbf117b/);
...
test('Apps Script evaluates the design template and includes both repository-owned fragments', () => {
  assert.match(code, /createTemplateFromFile\('Index'\)\.evaluate\(\)/);
...
test('known Claude demo records are absent from the live cockpit', () => {
  ...
  for (const demo of [
    ...
    'Inbox auf Null', 'Token expired', 'Woche vor der Woche'
